...
function listPluginDockEntries(): Array<{ id: ChannelId; dock: ChannelDock; order?: number }> {
  ...
  for (const entry of registry.channels) {
    ...
    seen.add(id);
    const chatId = plugin.id as ChatChannelId;
    const hasCoreDock = CHAT_CHANNEL_ORDER.includes(chatId) && Boolean(DOCKS[chatId]);
    if (hasCoreDock) {
      continue;
    }
...
export function listChannelDocks(): ChannelDock[] {
  ...
  return combined.map((entry) => entry.dock).filter((dock): dock is ChannelDock => Boolean(dock));
}
